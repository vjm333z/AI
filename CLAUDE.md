...
[STT]   kt-call-bot → POST /api/recording (python-svc)
    → Groq Whisper STT → 호텔 매칭(phone_lookup/hotels.json) → 요약 LLM(OpenAI)
...
- `service/HotelCacheService.java` / `service/PhoneLookupService.java` — `hotels.json` / `phone_lookup.json` 캐시 + 자동 추가

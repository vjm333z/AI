...
- LLM: Groq `llama-3.3-70b-versatile` (답변 메인) · OpenAI `gpt-4o-mini` (HyDE 메인 + 답변 폴백)
...
- `service/IndexService.java` — 적재 파이프라인 (indexAll / indexUpdated / indexSingle / indexAllTemplated / retryFailed / failedSummary). 무거운 작업은 내부 AtomicBoolean으로 동시 실행 차단.
- `service/SearchService.java` — 검색·답변 파이프라인 (ask / searchOnly / analyzeCategories + Reranker·MMR)
...
- `service/OpenAiService.java` — OpenAI Chat Completions (`ask` 폴백 + `templatize`, bean name `openai`)
- `service/TemplatizeService.java` — HyDE 추출기 인터페이스 (Groq/OpenAI 공통)
...
- `resources/application.yml` — Ollama / Qdrant / Groq / OpenAI + `rag.*` 파이프라인 튜닝
...
- `rag.templatize.provider` — `groq` | `openai` (HyDE 적재용)
...
- **시크릿 분리**: `application.yml`은 환경변수(`${GROQ_API_KEY}`, `${OPENAI_API_KEY}` 등)로만 주입. 평문 키 커밋 금지.
...
- **한국어 답변 고정**: LLM 프롬프트에서 한자·일본어·중국어 사용 금지, "처리 완료"·"해결 완료" 같은 단정 표현 금지 (`GroqService` / `OpenAiService` system prompt).

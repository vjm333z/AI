...
public class GroqService implements TemplatizeService {
    ...
    public String ask(String question, List<Map<String, Object>> similarCases) throws Exception {
        // 사례가 하나도 없으면 LLM 호출 건너뛰기 — 정확하고 빠른 "관련 사례 없음" 응답
        ...
        for (int i = 0; i < similarCases.size(); i++) {
            ...
            double score = ((Number) caseData.get("score")).doubleValue();
            Object rerankScore = caseData.get("rerank_score");
            ...
            if (rerankScore instanceof Number) {
                context.append(" | Reranker 관련도: ")
                       .append(String.format("%.1f%%", ((Number) rerankScore).doubleValue() * 100));
            }
            context.append("\n");
    ...
    public String proposeCategories(List<Map<String, String>> samples) throws Exception {
        ...
        for (int i = 0; i < samples.size(); i++) {
            Map<String, String> s = samples.get(i);
            String title = s.getOrDefault("title", "");
            String report = s.getOrDefault("report", "");
            userPrompt.append("[").append(i + 1).append("] ")
                      .append("제목: ").append(title).append("\n")
                      .append("    내용: ").append(report).append("\n\n");
        }
    ...
    public Map<String, Object> templatize(String report, String feedback) {
        ...
        Map<String, Object> systemMessage = new HashMap<>();
        systemMessage.put("role", "system");
        systemMessage.put("content", TEMPLATIZE_SYSTEM_PROMPT);
